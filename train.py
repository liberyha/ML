...
class NpyDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        #  找到目标类别对应的像素坐标，计算边界框的左下角和右上角坐标。
        y_indices, x_indices = np.where(gt2D > 0)
        x_min, x_max = np.min(x_indices), np.max(x_indices)
        y_min, y_max = np.min(y_indices), np.max(y_indices)
        # 根据边界框的扰动范围对边界框的坐标进行随机扰动。random.randint(0, self.bbox_shift) 生成一个介于0和 self.bbox_shift 之间的随机整数，用于在边界框的坐标上添加或减去这个随机值。max 和 min 函数确保扰动后的坐标不会超出图像的边界。
        H, W = gt2D.shape  # 大概也是为了增强模型泛化能力。加入随机扰动后使训练好的模型对于一些稍有偏差的提示信息也能保证分割的准确性。
        x_min = max(0, x_min - random.randint(0, self.bbox_shift))
        x_max = min(W, x_max + random.randint(0, self.bbox_shift))
        y_min = max(0, y_min - random.randint(0, self.bbox_shift))
        y_max = min(H, y_max + random.randint(0, self.bbox_shift))
        #  将边界框的坐标保存到数组 bboxes 中
        bboxes = np.array([x_min, y_min, x_max, y_max])  #  使用边界框的好处？简单，易储存，指示性强。
        return (
...
parser.add_argument(
    "-checkpoint", type=str, default="work_dir/MedSAM/medsam_vit_b.pth"  # 指定检查节点
)
...
parser.add_argument("-pretrain_model_path", type=str, default="./work_dir/MedSAM/medsam_vit_b.pth")  # 预训练模型路径
...
parser.add_argument("-use_amp", action="store_true", default=True, help="use amp")  # 不启用AMP（自动混合精度），AMP是一种在深度学习训练中用于提高性能和减少内存使用的技术，它结合了16位和32位浮点数的计算，以优化训练速度和效率。
