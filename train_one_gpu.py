...
class NpyDataset(Dataset):
    def __init__(self, data_root, bbox_shift=20):
        ...
        #  筛选出在图像路径下也存在相应文件名的标签文件，确保标签文件与图像文件对应。
        self.gt_path_files = [
            file
    ...
    def __getitem__(self, index):
        ...
        #  找到目标类别对应的像素坐标，计算边界框的左上角和右下角坐标。
        y_indices, x_indices = np.where(gt2D > 0)
        x_min, x_max = np.min(x_indices), np.max(x_indices)
        y_min, y_max = np.min(y_indices), np.max(y_indices)
        # 根据边界框的扰动范围对边界框的坐标进行扰动
        H, W = gt2D.shape
...
parser.add_argument(
    "-checkpoint", type=str, default="work_dir/SAM/sam_vit_b_01ec64.pth"  # 指定检查节点
)
# parser.add_argument('-device', type=str, default='cuda:0')  # 如果没有可用的 GPU，则会使用 CPU
parser.add_argument(
    "--load_pretrain", type=bool, default=True, help="use wandb to monitor training"  # 指示是否加载预训练模型，默认为 True
)
parser.add_argument("-pretrain_model_path", type=str, default="")  # 预训练模型路径
...
parser.add_argument("-batch_size", type=int, default=2)  # 批量大小
parser.add_argument("-num_workers", type=int, default=0)  # 工作线程
# 添加优化器参数
parser.add_argument(
    "-weight_decay", type=float, default=0.01, help="weight decay (default: 0.01)"  # 权重衰减
)
parser.add_argument(
    "-lr", type=float, default=0.0001, metavar="LR", help="learning rate (absolute lr)"  # 学习率
)
parser.add_argument(  # 监视训练参数
